  ...
  const switchTo = useCallback(
    async (convId: string) => {
      ...
      try {
        ...
        const messages: ChatMessage[] = detail.messages.map(
          (m: MessageData) => {
            ...
            // Parse [PLAN_CREATE] or legacy [TOOL_CALLS]create_plan[ARGS] → populate toolCalls
            {
              ...
              if (m.content.includes(planCreateTag)) {
                ...
                planArgsStr = m.content.substring(
                  markerIdx + planCreateTag.length,
                );
              } else if (m.content.includes(legacyTag)) {
                markerIdx = m.content.indexOf(legacyTag);
                planArgsStr = m.content.substring(
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
              ...
              if (planArgsStr !== null) {
                try {
                  ...
                  msg.content =
                    markerIdx > 0
                      ? m.content.substring(0, markerIdx).trim()
                      : "";
                } catch {
                  /* ignore malformed */
                }
            ...
            if (m.content.includes("[PLAN_COMPLETE]")) {
              try {
                const planJson = m.content.substring(
                  m.content.indexOf("[PLAN_COMPLETE]") +
                    "[PLAN_COMPLETE]".length,
                );
                const planData = JSON.parse(planJson.trim());
                if (planData.goal && planData.steps) {
                  msg.toolCalls = [
                    {
                      name: "create_plan",
                      arguments: { goal: planData.goal, steps: planData.steps },
                    },
                  ];
                }
              } catch {
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
        ...
        const hasPlan = messages.some((m) =>
          m.toolCalls?.some((tc) => tc.name === "create_plan"),
        );
        if (hasPlan) {
  ...
  const createNew = useCallback(async () => {
    try {
      ...
      chatDispatch({
        type: "SET_CONVERSATION",
        payload: { id: newConv.id, messages: [] },
      });
    } catch {
      chatDispatch({
        type: "SET_CONVERSATION",
        payload: { id: null, messages: [] },
      });
    }
  ...
  function restoreDetailPanel(messages: MessageData[]) {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.role === "assistant" && msg.content.includes("[PLAN_COMPLETE]")) {
        try {
          ...
          console.log("[restoreDetailPanel] Found [PLAN_COMPLETE]:", {
            goal: planData.goal,
            stepsCount: planData.steps?.length,
            resultsCount: planData.results?.length,
          });
          if (planData.goal && planData.steps) {
            const isStopped = !!planData.stopped;
            const completedSteps = new Set(
              (planData.results || [])
                .filter((r: { success: boolean; step: number }) => r.success)
                .map((r: { step: number }) => r.step),
            );
    ...
    console.log(
      "[restoreDetailPanel] No [PLAN_COMPLETE] found, trying [PLAN_CREATE]...",
    );
    for (let i = messages.length - 1; i >= 0; i--) {
      const msg = messages[i];
      const hasPlanCreate =
        msg.role === "assistant" &&
        (msg.content.includes("[PLAN_CREATE]") ||
          msg.content.includes("[TOOL_CALLS]create_plan[ARGS]"));
      if (hasPlanCreate) {
        try {
          let argsStr: string;
    ...
    console.log(
      "[restoreDetailPanel] No plan markers found in any message. Messages summary:",
      messages.map((m) => ({
        role: m.role,
