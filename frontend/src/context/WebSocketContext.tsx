    ...
    (raw: unknown) => {
      ...
      switch (eventType) {
        case "token": {
          ...
          const token =
            (eventData.token as string) ?? (event.token as string) ?? "";
        ...
        case "tool_call": {
          const toolCall =
            (eventData.tool_call as Record<string, unknown>) ??
            (event.tool_call as Record<string, unknown>) ??
            eventData;
          chatDispatch({
            type: "ADD_TOOL_CALL",
            payload: toolCall as ToolCallEvent["tool_call"],
          });
          ...
          if (toolCall.name === "create_plan") {
            ...
            if (args?.goal && args?.steps) {
              console.log(
                "[WS] Initializing detailPanelData:",
                args.steps.length,
                "steps",
              );
        ...
        case "tool_result": {
          const toolResult =
            (eventData.tool_result as Record<string, unknown>) ??
            (event.tool_result as Record<string, unknown>) ??
            eventData;
          console.log("[WS] tool_result:", {
          ...
          chatDispatch({
            ...
            payload: toolResult as ToolResultEvent["tool_result"],
          });
          ...
          if (step !== undefined) {
            ...
            if (toolResult.tool) {
              appDispatch({
                ...
                payload: {
                  stepIndex: stepIdx,
                  toolName: toolResult.tool as string,
                },
              });
            }
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
        ...
        case "tool_retrieval_start": {
          appDispatch({
            type: "SET_TOOL_RETRIEVAL_STATUS",
            payload: "running",
          });
          break;
        }
        ...
        case "tool_retrieval_done": {
          ...
          const trDone = eventData.tool_retrieval_done as
            | Record<string, unknown>
            | undefined;
        ...
        case "step_execute": {
          const stepExec =
            (eventData.step_execute as Record<string, unknown>) ?? eventData;
          appDispatch({
        ...
        case "step_start": {
          ...
          const stepStart =
            (eventData.step_start as Record<string, unknown>) ??
            (event.step_start as Record<string, unknown>) ??
            eventData;
          const stepNum = stepStart.step as number;

          // Auto-complete all previous running steps (mirrors original inference_ui behavior)
          appDispatch({
            type: "COMPLETE_PREVIOUS_RUNNING_STEPS",
            payload: stepNum - 1,
          });
          ...
          const retrievedTools = stepStart.retrieved_tools as
            | string[]
            | undefined;
          if (retrievedTools && retrievedTools.length > 0 && stepNum === 1) {
            appDispatch({
              type: "SET_RETRIEVED_TOOLS",
              payload: retrievedTools,
            });
          }
          break;
        }
        ...
        case "done": {
          ...
          const stopped =
            (eventData.stopped as boolean) ?? (event.stopped as boolean);
          if (stopped) {
            appDispatch({ type: "MARK_RUNNING_STEPS_STOPPED" });
          }
          const planComplete =
            (eventData.plan_complete as Record<string, unknown>) ??
          ...
          if (planComplete) {
            ...
            chatDispatch({
              type: "SET_PLAN_COMPLETE",
              payload: planComplete as unknown as PlanComplete,
            });
            // Populate codes in detail panel for Code tab
            const codes = planComplete.codes as
              | Record<string, unknown>
              | undefined;
            if (codes) {
              for (const [key, val] of Object.entries(codes)) {
                ...
                if (typeof val === "string") {
                  appDispatch({
                    type: "SET_STEP_CODE",
                    payload: { stepIndex: idx, code: val },
                  });
