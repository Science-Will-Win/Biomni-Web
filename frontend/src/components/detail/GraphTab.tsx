...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
    ...
    const planKey = JSON.stringify(
      data.steps.map((s) => `${s.name}|${s.description}`),
    );
    ...
    if (restoredRef.current) {
      ...
      const namesMatch =
        restoredSteps.length === data.steps.length &&
    ...
    const userNodes = [...eng.state.nodes.values()].filter(
      (n) => !planManagedIds.has(n.id),
    );
    const userConnections = [...eng.state.connections.values()].filter(
      (c) => !planManagedIds.has(c.from) || !planManagedIds.has(c.to),
    );
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
  ...
  const hasExecutionLogicChanged = useCallback(() => {
    const newHash = getExecutionPlanHash(
      engine.state.nodes,
      engine.state.connections,
    );
  ...
  useEffect(() => {
    ...
    return () => {
      delete (window as unknown as Record<string, unknown>).__graphEngine;
    };
  ...
  const handleRerunPlan = useCallback(() => {
    ...
    // Update hash
    lastPlanHashRef.current = getExecutionPlanHash(
      engine.state.nodes,
      engine.state.connections,
    );
  }, [
    ...
    <div className="graph-tab-wrapper">
      <div className="graph-tab-header">
        ...
        <div className="graph-tab-header-actions">
          <button
            className="graph-popout-btn"
            onClick={() => setIsPopout(true)}
            title={t("tooltip.popout")}
          >
            <svg
              width="14"
              height="14"
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
          ...
          <div className="graph-popout-notice">
            <span>
              {t("empty.graph_popout") || "Graph is open in a separate window"}
            </span>
            <button onClick={() => setIsPopout(false)}>
              {t("label.close") || "Close"}
            </button>
          </div>
        </>
