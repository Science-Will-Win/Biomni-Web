...
type GraphAction =
  ...
  | {
      type: "SET_PORT_VALUE";
      payload: { nodeId: string; portName: string; value: unknown };
    }
  ...
  switch (action.type) {
    ...
    case "SET_PORT_VALUE": {
      ...
      if (node) {
        const portValues = {
          ...node.portValues,
          [action.payload.portName]: action.payload.value,
        };
    ...
    case "ADD_CONNECTION": {
      ...
      if (conn.type === "ref") {
        ...
        if (fromNode?.stepNum && toNode?.stepNum) {
          if (parseFloat(fromNode.stepNum) >= parseFloat(toNode.stepNum))
            return state;
        }
      }
      ...
      if (conn.type === "flow") {
        ...
        if (fromNode && toNode) {
          ...
          const fromPortDef = fromDef?.ports.find(
            (p) => p.name === conn.fromPort,
          );
          const toPortDef = toDef?.ports.find((p) => p.name === conn.toPort);
          if (
            fromPortDef &&
            toPortDef &&
            !PortTypes.isCompatible(fromPortDef.type, toPortDef.type)
          ) {
            return state;
          }
        }
...
export function useGraphEngine() {
  ...
  const setPortValue = useCallback(
    (nodeId: string, portName: string, value: unknown) => {
      dispatch({
        type: "SET_PORT_VALUE",
        payload: { nodeId, portName, value },
      });
    },
    [],
  );
  ...
  const addConnection = useCallback(
    (
      ...
      dispatch({
        type: "ADD_CONNECTION",
        payload: { id, from, fromPort, to, toPort, type },
      });
      return id;
    },
    [],
