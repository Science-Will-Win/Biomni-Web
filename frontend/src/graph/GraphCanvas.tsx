  ...
  return (
    <div
      ...
      style={{
        ...
        left: node.x,
        top: node.y,
        width: node.width || def.minWidth || 180,
    ...
    >
      ...
      {(selected || multiSelected) &&
        RESIZE_HANDLES.map((h) => (
          <div
            key={h}
            className={`ng-resize-handle ng-resize-${h}`}
            onMouseDown={(e) => {
              e.stopPropagation();
              onResizeStart(e, node.id, h);
            }}
          />
        ))}
    </div>
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
  ...
  useEffect(() => {
    if (nodesArray.length === 0) hasFitted.current = false;
  }, [nodesArray.length]);
...
>>>>>>> 064c1ba3e0e3069e5c3e5d438c7fb44144593902
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const scrollable =
        target.closest(".ng-node-body, .ng-input-node-body") ||
      ...
      engine.setViewport(
        mx - (mx - engine.state.panX) * (newS / oldS),
        my - (my - engine.state.panY) * (newS / oldS),
        newS,
      );
    };
  ...
  const highlightPorts = useCallback(
    (pending: PendingConn) => {
      const container = containerRef.current;
      if (!container) return;
      ...
      container.querySelectorAll<HTMLElement>(".ng-port").forEach((portEl) => {
        ...
        if (pending.connType === "ref") {
          ...
          const pendingDef = getNodeDef(
            state.nodes.get(pending.fromNodeId)?.type || "",
          );
  ...
  const clearPortHighlights = useCallback(() => {
    containerRef.current
      ?.querySelectorAll(".ng-port-compatible, .ng-port-dimmed, .ng-port-snap")
      .forEach((el) => {
        el.classList.remove(
          "ng-port-compatible",
          "ng-port-dimmed",
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (pending) {
        ...
        const rect = container.getBoundingClientRect();
        const mx =
          (e.clientX - rect.left - engine.state.panX) / engine.state.scale;
        const my =
          (e.clientY - rect.top - engine.state.panY) / engine.state.scale;
      ...
      if (drag.type === "pan") {
        ...
        const gy =
          (e.clientY - rect.top - engine.state.panY) / engine.state.scale;
        const mx = Math.min(drag.startPanX, gx),
          my = Math.min(drag.startPanY, gy);
        const mw = Math.abs(gx - drag.startPanX),
          mh = Math.abs(gy - drag.startPanY);
    ...
    (
      ...
      if (portDir === "in") {
        ...
        const selectedConn = state.selectedConnectionId
          ? state.connections.get(state.selectedConnectionId)
          : null;
        if (
          selectedConn &&
          selectedConn.to === nodeId &&
          selectedConn.toPort === portName
        ) {
          removeConnection(selectedConn.id);
          const fromNode = state.nodes.get(selectedConn.from);
          if (fromNode) {
            const center = getPortCenter(
              fromNode,
            ...
            const p: PendingConn = {
              fromNodeId: selectedConn.from,
              fromPort: selectedConn.fromPort,
              fromDir: "out",
              fromType: portType,
      ...
      const canStartRef = nodeDef
        ? !noRefCategories.includes(nodeDef.category)
        : true;
      const connType: ConnectionType =
        isRightClick && canStartRef ? "ref" : "flow";
  ...
  const handleContainerMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (e.button === 0) {
        const target = e.target as HTMLElement;
        if (
          target.closest(".ng-node") ||
          target.closest(".ng-create-menu") ||
          target.closest(".ng-zoom-controls")
        )
          return;
  ...
  const handleCreateNode = useCallback(
    (type: string) => {
      ...
      engine.addNode({
        ...
        portValues: def.defaultConfig.portValues
          ? { ...def.defaultConfig.portValues }
          : undefined,
      });
      setCreateMenu(null);
    },
  ...
  const handleNodeMouseDown = useCallback(
    (e: React.MouseEvent, nodeId: string) => {
      ...
      const selectedIds = state.selectedNodeIds.has(nodeId)
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "v") {
        ...
        for (const n of clip.nodes) {
          ...
          if (n.x + (n.width || 180) > clipMaxX)
            clipMaxX = n.x + (n.width || 180);
          if (n.y + (n.height || 80) > clipMaxY)
            clipMaxY = n.y + (n.height || 80);
        }
        ...
        const container = containerRef.current;
        let targetX = clipCenterX + 30,
          targetY = clipCenterY + 30;
        if (container) {
          const vc = getViewportCenterGraphPos(
            container,
            state.panX,
            state.panY,
            state.scale,
  ...
  const handlePortValueChange = useCallback(
    (nId: string, pName: string, val: unknown) => {
      setPortValue(nId, pName, val);
    },
    [setPortValue],
  );
    ...
    <GraphProvider value={graphCtx}>
      ...
      >
        ...
        >
          {connectionsArray.map((conn) => (
            <ConnectionLine
              ...
              selected={conn.id === state.selectedConnectionId}
          ...
          {pendingLine && (
            <PendingConnectionLine
              ...
              x2={pendingLine.x2}
              y2={pendingLine.y2}
              isRef={pendingLine.isRef}
            />
          )}
        </svg>
        {/* Viewport: 노드만 포함 */}
        <div
          className="node-graph-viewport"
          style={{
            transform: `translate(${state.panX}px, ${state.panY}px) scale(${state.scale})`,
            transformOrigin: "0 0",
            opacity: layoutReady ? 1 : 0,
        ...
        <div className="ng-zoom-controls">
          <button
            className="ng-zoom-btn"
            onClick={() =>
              setViewport(
                state.panX,
                state.panY,
                Math.min(3, state.scale * 1.2),
              )
            }
            title="Zoom In"
          >
          ...
          <button
            className="ng-zoom-btn"
            onClick={() =>
              containerRef.current && fitToView(engine, containerRef.current)
            }
            title="Reset Zoom"
          >
            ⌂
          ...
          <button
            className="ng-help-btn"
            onClick={() =>
              window.open("https://github.com/your-repo/biomni-web", "_blank")
            }
            title="Help"
          >
            ?
...
function findSnapPort(
  pending: PendingConn,
  mx: number,
  my: number,
  gs: { nodes: Map<string, NodeData> },
  container: HTMLElement | null,
): { nodeId: string; portName: string; x: number; y: number } | null {
  const SNAP = 15;
  const targetDir = pending.fromDir === "out" ? "in" : "out";
  let best: { nodeId: string; portName: string; x: number; y: number } | null =
    null;
  let bestDist = SNAP;
  for (const [nId, node] of gs.nodes) {
    ...
    for (const p of portsToCheck) {
      ...
      if (pending.connType === "ref") {
        ...
        const pendingDef = getNodeDef(
          gs.nodes.get(pending.fromNodeId)?.type || "",
        );
      ...
      if (d < bestDist) {
        bestDist = d;
        best = { nodeId: nId, portName: p.name, x: c.x, y: c.y };
      }
    }
  }
  return best;
...
function fitToView(
  ...
  const s = Math.max(
    0.2,
    Math.min(1, Math.min((cw - padding * 2) / cW, (ch - padding * 2) / cH)),
  );
  engine.setViewport(
    (cw - cW * s) / 2 - minX * s,
    (ch - cH * s) / 2 - minY * s,
